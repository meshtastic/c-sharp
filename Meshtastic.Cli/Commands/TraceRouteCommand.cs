...
{
    public TraceRouteCommand(string name, string description, Option<string> port, Option<string> host,
        ...
        base(name, description)
    {
        this.SetHandler(async (context, commandContext) =>
            {
                var handler = new TraceRouteCommandHandler(context, commandContext);
